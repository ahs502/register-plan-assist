    ...
    this.preplan.stagedFlightRequirements.forEach(r => {
      if (this.data.airports.includes(r.definition.departureAirport) || this.data.airports.includes(r.definition.arrivalAirport)) {
        ...
        if (commonCount === r.scope.aircraftSelection.aircraftRegisters.length)
          return objections.push(r.issueObjection(this.data.required ? 'ERROR' : 'WARNING', 12345, this, constraintMarker => `${r.marker} violates ${constraintMarker}.`));
        if (commonCount > 0) return objections.push(r.issueObjection('WARNING', 12345, this, constraintMarker => `${r.marker} may violate ${constraintMarker}.`)); //TODO: Refine this instantiation.
      }
      r.days.forEach(d => {
        if (this.data.airports.includes(d.definition.departureAirport) || this.data.airports.includes(d.definition.arrivalAirport)) {
          ...
          if (commonCount === d.scope.aircraftSelection.aircraftRegisters.length)
            return objections.push(d.issueObjection(this.data.required ? 'ERROR' : 'WARNING', 12345, this, constraintMarker => `${d.marker} violates ${constraintMarker}.`));
          if (commonCount > 0) return objections.push(d.issueObjection('WARNING', 12345, this, constraintMarker => `${d.marker} may violate ${constraintMarker}.`));
        }
      });
    });
    return objections;
