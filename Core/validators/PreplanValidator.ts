...
export default class PreplanValidator {
  ...
  static addOrEditFlightRequirementValidate(flightRequirement: any, dummyAircraftRegisters: readonly DummyAircraftRegisterModel[]) {
    const validation = new Validation<
      ...
      | 'WEEKDAY_NOTES_IS_VALID'
      | 'WEEKDAY_DAY_IS_VALID'
      | 'WEEKDAY_FLIGHT_STD_EXISTS'
      | 'WEEKDAY_FLIGHT_STD_IS_VALID'
      | 'WEEKDAY_FLIGHT_STD_IS_NOT_NEGATIVE'
      | 'WEEKDAY_FLIGHT_AIRCRAFT_REGISTER_ID_IS_VALID'
      | 'DAYS_ARE_UNIQUE'
      | 'IGNORED_IS_VALID'
    >();
    validation.if(flightRequirement && typeof flightRequirement === 'object').do(() => {
      ...
      validation.if(definition && typeof definition === 'object').do(() => {
        const { label, stcId, flightNumber, departureAirportId, arrivalAirportId } = definition;
        validation.check('LABEL_EXISTS', label && typeof label === 'string', 'Required.');
        validation.check('STC_EXISTS', stcId && typeof stcId === 'string', 'Required.').check('STC_IS_VALID', () => !!MasterData.all.stcs.id[stcId], 'Invalid.');
        validation
            ...
            allowedIdentities.forEach((identity, index) => {
              validation.if(identity && typeof identity === 'object').do(() => {
                ...
                validation
                  ...
                  .check('ALLOWED_IDENTITIES_TYPE_IS_VALID', () => checkAircraftIdentityTypeValidity(type), 'Invalid.', index);
                validation
            ...
            forbiddenIdentitied.forEach((identity, index) => {
              validation.if(identity && typeof identity === 'object').do(() => {
                ...
                validation
                  ...
                  .check('FORBIDDEN_IDENTITIES_TYPE_IS_VALID', () => checkAircraftIdentityTypeValidity(type), 'Invalid.', index);
                validation
      ...
      validation
        ...
        .do(() => {
          days.forEach((weekdayFlightRequirement, index) => {
            validation.if(weekdayFlightRequirement && typeof weekdayFlightRequirement === 'object').do(() => {
                ...
                validation.if(times && Array.isArray(times)).do(() => {
                  times.forEach((time, subIndex) => {
                    validation.if(time && typeof time === 'object').do(() => {
                      ...
                      validation
                        .when(
                          { badge: 'WEEKDAY_TIMES_STD_LOWER_BOUND_IS_NOT_NEGATIVE', index, subIndex },
                          { badge: 'WEEKDAY_TIMES_STD_UPPER_BOUND_IS_NOT_NEGATIVE', index, subIndex }
                        )
                        .check(
                          'WEEKDAY_TIMES_STD_LOWER_BOUND_BEFORE_STD_UPPER_BOUND',
                          () => stdLowerBound <= stdUpperBound,
                          'Lower bound can not exceed upper bound.',
                          index,
                          subIndex
                        );
                    });
                    ...
                    allowedIdentities.forEach((identity, subIndex) => {
                      validation.if(identity && typeof identity === 'object').do(() => {
                        ...
                        validation
                          ...
                          .check('WEEKDAY_ALLOWED_IDENTITIES_TYPE_IS_VALID', () => checkAircraftIdentityTypeValidity(type), 'Invalid.', index, subIndex);
                        validation
                    ...
                    forbiddenIdentitied.forEach((identity, subIndex) => {
                      validation.if(identity && typeof identity === 'object').do(() => {
                        ...
                        validation
                          ...
                          .check('WEEKDAY_FORBIDDEN_IDENTITIES_TYPE_IS_VALID', () => checkAircraftIdentityTypeValidity(type), 'Invalid.', index, subIndex);
                        validation
              ...
              validation.check('WEEKDAY_NOTES_IS_VALID', typeof notes === 'string', 'Invalid.', index);
              validation.check('WEEKDAY_DAY_IS_VALID', typeof day === 'number' && !isNaN(day) && day >= 0 && day < 7, 'Invalid.', index);
              validation.if(flight && typeof flight === 'object').do(() => {
                const { std, aircraftRegisterId } = flight;
                validation
                  .check('WEEKDAY_FLIGHT_STD_EXISTS', std || std === 0 || isNaN(std), 'Required.', index)
                  .check('WEEKDAY_FLIGHT_STD_IS_VALID', () => !isNaN(std), 'Invalid.', index)
                  .check('WEEKDAY_FLIGHT_STD_IS_NOT_NEGATIVE', () => std > 0, 'Should be greater than 0.', index);
                validation.check(
                  'WEEKDAY_FLIGHT_AIRCRAFT_REGISTER_ID_IS_VALID',
                  aircraftRegisterId === undefined || !!MasterData.all.aircraftRegisters.id[aircraftRegisterId] || dummyAircraftRegisters.some(a => a.id === aircraftRegisterId),
                  'Invalid.',
                  index
                );
              });
        ...
        .when(Array.range(0, days.length - 1).map(index => ({ badge: 'WEEKDAY_DAY_IS_VALID', index })))
        .check('DAYS_ARE_UNIQUE', () => days.map(d => d.day).distinct().length === days.length, 'Weekday duplicates.');
      validation.check('IGNORED_IS_VALID', typeof ignored === 'boolean', 'Invalid.');
    });
    return validation;

    function checkAircraftIdentityTypeValidity(type: string): boolean {
      return ['REGISTER', 'TYPE', 'TYPE_EXISTING', 'TYPE_DUMMY', 'GROUP'].includes(type);
    }
